...
class FormatAppError {
    operator fun invoke(error: AppError): String =
        when (error) {
            is AppError.Multi -> formatMultiError(error)
            is AppError.Remote -> formatRemoteError(error)
            is AppError.Database.Io ->
                "Database error: ${error.cause.message ?: UNKNOWN_ERROR_MESSAGE}"
    ...
    private fun formatRemoteError(error: AppError.Remote): String =
        when (error) {
            is AppError.Remote.Http ->
                "HTTP ${error.code}: ${error.httpMessage ?: UNKNOWN_ERROR_MESSAGE}"
