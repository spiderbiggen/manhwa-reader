...
sealed class AppError(override val message: String? = null, override val cause: Throwable? = null) :
    ...
    data class Multi(val errors: NonEmptyList<AppError>) :
        AppError(message = errors.joinToString("\n") { it.message ?: it.toString() })

    sealed class Remote(message: String? = null, cause: Throwable? = null) :
        ...
        data class NotFound(override val cause: Throwable? = null) :
            Remote("Resource Not Found", cause)

        data class NoConnection(override val cause: Throwable? = null) :
            Remote("No Connection", cause)
    ...
    sealed class Database(message: String? = null, cause: Throwable? = null) :
        AppError(message, cause) {
