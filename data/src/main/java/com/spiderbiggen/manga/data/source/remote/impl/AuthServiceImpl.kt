...
class AuthServiceImpl(private val client: HttpClient) : AuthService {
    ...
    override suspend fun register(body: RegisterBody): SessionResponse =
        client
    ...
    override suspend fun login(body: LoginBody): SessionResponse =
        client.post("api/v1/auth/login") { setBody(body) }.body()

    override suspend fun refresh(body: RefreshTokenBody): SessionResponse =
        client
